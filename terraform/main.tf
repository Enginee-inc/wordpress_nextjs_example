...
resource "azurerm_linux_web_app" "wordpress_web_app" {
  ...
  site_config {
    ...
    application_stack {
      docker_registry_url = "https://${var.app_service_docker_registry_url}"
      docker_image_name = "${var.wordpress_container_linux_fx_version}"
    }
  }
...
module "static_web_app" {
  ...
  tags                = var.tags
  repository_url = var.static_web_app_repository_url
  repository_branch = var.static_web_app_repository_branch
  repository_token = var.static_web_app_repository_token
  ...
  depends_on = [
    azurerm_linux_web_app.wordpress_web_app
  ]
}
